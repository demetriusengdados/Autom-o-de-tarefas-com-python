...
def cc_generator():
	...
	print("Cartões suportados:\n************************")
	for i in cc:
		print( a, ">", i)
		a+=1

	ccn = int(input("Escolha o número do cartão de crédito na lista acima: [NOTA: Pressione -1 se você deseja gerar cc aleatório]::"))

	if(ccn == -1):
		print("Escolha o número total de cartões de crédito aleatórios que você deseja gerar:" , end = '')
		cca = int(input())
		print("*************************** A lista de números / nomes de cartão de crédito gerados aleatoriamente é mostrada abaixo ******************************")
		ccn_r = [1 , 2, 3, 4 , 5]
		...
		if not( 1 <= ccn <= 5 ):
			print('wrong choice')
			return
		print("Escolha o número total de cartões de crédito que você deseja gerar para " , cc[ccn-1]  , ":" , sep = '' , end='')
		cca = int(input())
		print("*************************** Lista de cartões gerados" , cc[ccn-1]  ,"os números são mostrados abaixo ******************************")
		print_cc(ccn , cca )
